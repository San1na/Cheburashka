...
function Library:AddTab(tabSettings)
    ...
    function tab:AddSection(sectionSettings)
        ...
        function api:AddDropdown(data)
            ...
            local baseHeight = data.Height or style.ItemHeight
            ...
            makeStroke(selectBg, style.BorderColor, 0)

            local selectBtn = makeButton(selectBg)
            selectBtn.Size = UDim2.fromScale(1, 1)
            ...
            local optionsStroke = makeStroke(optionsFrame, style.BorderColor, 1)
            ...
            optionsLayout.Padding = UDim.new(0, optionGap)
            optionsLayout.Parent = optionsFrame

            local function getOptionsHeight()
                local count = 0
                for _ in pairs(optionRows) do
                    count = count + 1
                end
                if count <= 0 then return 0 end
                return 8 + (count * optionHeight) + ((count - 1) * optionGap)
            end

            local function refreshVisuals()
                for name, ref in pairs(optionRows) do
                    local active = (name == selected)
                    tween(ref.Text, 0.12, { TextColor3 = active and style.TextColor or style.SubTextColor })
                    tween(ref.Button, 0.12, { BackgroundTransparency = active and 0.85 or 1 })
                end
            ...
            local function setSelected(nextValue, silent)
                local value = tostring(nextValue)
                if optionRows[value] == nil then return end
                selected = value
                refreshVisuals()
                if data.Callback then data.Callback(selected) end
                if not silent and controlRef then
                    menuRef:_emitChanged(controlRef.Flag)
                end
            end
            ...
            local function rebuild(newOptions)
                clearOptions()
                local seen = {}
                for _, name in ipairs(normalizeOptions(newOptions)) do
                    if not seen[name] then
                        seen[name] = true
                        local btn = Instance.new("TextButton")
                        btn.AutoButtonColor = false
                        btn.Text = ""
                        btn.BackgroundColor3 = style.ItemColor
                        btn.BackgroundTransparency = 1
                        btn.Size = UDim2.new(1, 0, 0, optionHeight)
                        btn.Parent = optionsFrame
                        makeCorner(btn, 4)

                        local txt = makeLabel(btn, name, style.SmallTextSize, style.SubTextColor, style.Font, Enum.TextXAlignment.Left)
                        ...
                        optionRows[name] = { Button = btn, Text = txt }
                    end
                end

                if optionRows[selected] == nil then
                    for name in pairs(optionRows) do
                        selected = name
                        break
                    end
            ...
            controlRef = menuRef:_registerControl({
                ...
                Set = function(v, silent) setSelected(v, silent == true) end,
            })
        ...
        function api:AddMultiBoolean(data)
            ...
            local function updateSummary()
                ...
                elseif count == 1 then
                    summaryLabel.Text = singleName
                else
                    summaryLabel.Text = tostring(count) .. " selected"
                end
            end

            local function updateOptionVisual(name)
                local ref = optionRows[name]
                if not ref then return end
                local active = states[name] == true
                tween(ref.Button, 0.12, { BackgroundTransparency = active and 0.85 or 1 })
                tween(ref.Text, 0.12, { TextColor3 = active and style.TextColor or style.SubTextColor })
            end
            ...
            local function setOption(option, state, silent)
                ...
                updateSummary()
                if data.Callback then data.Callback(name, states[name], getStatesCopy()) end
                if not silent and controlRef then
                    menuRef:_emitChanged(controlRef.Flag)
                end
            end
            ...
            local function setAll(nextStates, silent)
                ...
                for name in pairs(states) do
                    if nextStates[name] ~= nil then
                        states[name] = nextStates[name] == true
                        updateOptionVisual(name)
                    end
                end
                updateSummary()
            ...
            local function rebuild(newOptions, defaults)
                ...
                for _, name in ipairs(normalizeOptions(newOptions)) do
                    if not seen[name] then
                        seen[name] = true
                        states[name] = typeof(defaults) == "table" and defaults[name] == true or false
                        ...
                        btn.MouseButton1Click:Connect(function()
                            setOption(name, not states[name], false)
                        end)
            ...
            controlRef = menuRef:_registerControl({
                ...
                Set = function(v, silent) setAll(v, silent == true) end,
            })
